#!/usr/bin/env python3
"""
BH evolution using JAX backend.

Evolves an isotropic Schwarzschild BH with scalar field matter
using BSSN equations, fully JIT-compiled via JAX.

Usage:
    ENGRENAGE_BACKEND=jax python examples/BH/BHEvolution_jax.py

Environment variables:
    ENGRENAGE_BACKEND: must be "jax" (or "jax-cpu" / "jax-gpu")
    T_FINAL: evolution time (default 10)
    NUM_POINTS: override grid points (default: auto from CubicSpacing)
"""
...
from source.initialdata.bhinitialconditions import get_initial_state
...
def main():
    print("=" * 60)
    print("  BH Evolution — JAX Backend")
    print("=" * 60)
    ...
    num_points_t = 128   # Output time resolution
    ...
    parity = np.concatenate([np.array(my_state_vector.PARITY, dtype=np.float64)])
    asymp_power = np.concatenate([np.array(my_state_vector.ASYMP_POWER, dtype=np.float64)])
    asymp_offset = np.concatenate([np.array(my_state_vector.ASYMP_OFFSET, dtype=np.float64)])

    # =========================================================================
    # Define RK4 step
    # =========================================================================
    from source.bssn.jax.boundaries_jax import fill_bssn_boundaries_jax
    from source.bssn.bssnstatevariables import BSSN_PARITY, BSSN_ASYMP_POWER, BSSN_ASYMP_OFFSET
    ...
    for step in range(1, n_steps + 1):
        # Adjust last step to land exactly on T_final
    ...
    # Ensure we block until the last computation is done
    ...
    # Convert outputs to the format expected by diagnostics
    output_times = np.array(output_times)
    # Stack states into (n_outputs, num_vars*N) flat format for diagnostics
    ...
    # AH finder
    print("  Finding apparent horizon...")
    try:
        omega, ah_radius, bh_mass = get_horizon_diagnostics(
            solution_flat, output_times, grid, background, my_matter)
        print(f"  AH radius at t=0:     {ah_radius[0]:.4f}")
        print(f"  AH radius at t=final: {ah_radius[-1]:.4f}")
        print(f"  BH mass at t=0:       {bh_mass[0]:.4f}")
        print(f"  BH mass at t=final:   {bh_mass[-1]:.4f}")
    except Exception as e:
        print(f"  AH finder failed: {e}")
    ...
    try:
        ...
        # (0,0) phi and lapse at inner point vs time
        ax = axes[0, 0]
        idx_point = NUM_GHOSTS + 2
        r_point = r[idx_point]
        ...
        # (0,1) lapse profile at several times
        ax = axes[0, 1]
        n_out = len(output_times)
        for i in range(0, n_out, max(1, n_out // 6)):
            t_i = output_times[i]
            lapse_profile = output_states[i][idx_lapse]
            ax.plot(r, lapse_profile, label=f't={t_i:.1f}')
        ax.set_xlabel('r')
        ax.set_ylabel('lapse')
        ax.set_xlim(0, 48)
        ax.legend()
        ax.grid(True)
        ax.set_title('Lapse profile')

        # (1,0) Hamiltonian constraint
        ax = axes[1, 0]
        for i in range(0, n_out, max(1, n_out // 5)):
            t_i = output_times[i]
        ...
        ax = axes[1, 1]
        if 'bh_mass' in dir():
            ax.plot(output_times, bh_mass, 'b-', label='AH Mass')
            ax.plot(output_times, ah_radius, 'g--', label='AH Radius')
            ax.set_xlabel('t')
            ax.set_ylabel('value')
            ax.legend()
            ax.grid(True)
            ax.set_ylim(-0.1, 1.5)
        ax.set_title('BH mass and AH radius')

        plt.tight_layout()
        plot_path = os.path.join(os.path.dirname(__file__), 'bh_evolution_jax.png')
    ...
    print("\n" + "=" * 60)
    print("  BH Evolution JAX — Complete")
    print("=" * 60)
