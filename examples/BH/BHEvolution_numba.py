#!/usr/bin/env python3
"""
BH evolution using Numba/NumPy backend (converted from BHEvolution.ipynb).

Evolves an isotropic Schwarzschild BH with scalar field matter
using BSSN equations with manual RK4 (fixed step).

Usage:
    python examples/BH/BHEvolution_numba.py
"""
...
def main():
    print("=" * 60)
    print("  BH Evolution — Numba/NumPy Backend")
    print("=" * 60)

    # =========================================================================
    # Configuration (same as BHEvolution.ipynb / BHEvolution_jax.py)
    # =========================================================================
    scalar_mu = 1.0
    r_max = 96.0
    min_dr = 1.0 / 16.0
    ...
    try:
        omega, ah_radius, bh_mass = get_horizon_diagnostics(
            solution, t_out, grid, background, my_matter)
    ...
    try:
        ...
        fig.suptitle(f"BH Evolution — Numba (T={T}, N={num_points})", fontsize=14)
        ...
        for i in range(0, n_out, max(1, n_out // 6)):
            lapse_profile = solution[i, idx_lapse*num_points:(idx_lapse+1)*num_points]
            ax.plot(r, lapse_profile, label=f't={t_out[i]:.1f}')
        ax.set_xlabel('r')
        ax.set_ylabel('lapse')
        ax.set_xlim(0, 48)
        ax.legend()
        ...
        ax = axes[1, 1]
        if 'bh_mass' in dir():
            ax.plot(t_out, bh_mass, 'b-', label='AH Mass')
            ax.plot(t_out, ah_radius, 'g--', label='AH Radius')
            ax.set_xlabel('t')
            ax.legend()
            ax.grid(True)
            ax.set_ylim(-0.1, 1.5)
        ...
        plt.tight_layout()
        plot_path = os.path.join(script_dir, 'bh_evolution_numba.png')
