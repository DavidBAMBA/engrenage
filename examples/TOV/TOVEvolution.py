...
def rk4_step(state_flat, dt, grid, background, hydro, bssn_fixed, bssn_d1_fixed, atmosphere):
    ...
    Following  : apply atmosphere floors after EACH RK substep.
    ...
    apply_floors_to_state(s2, grid, hydro)  #   / -style floors
    state_2 = s2.flatten()
    ...
    # (now only touches physical cells, not ghosts)
...
def main():
    ...
    OUTPUT_DIR = os.path.join(plots_dir, "tov_evolution_data2")  # Directory for output data
