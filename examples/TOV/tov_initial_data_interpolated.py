...
# Import cons2prim conversion function
...
def convert_adm_to_bssn(adm_vars, grid, background):
    ...
    8. Set gauge variables: α (from TOV), λ^r=0, β^r=0, b^r=0

    Note: For static TOV, λ̄^i = 0, β^i = 0, b^i = 0 are the correct initial values.
          During evolution, boundary conditions and gauge choices will adjust these.
    ...
    for i in range(N):
        conformal_factor_3d = (det_ghat[i] / max(det_gamma[i], 1e-30)) ** (1.0/3.0)
        for j in range(3):
            for k in range(3):
    ...
    # Static TOV: zero shift, zero connections
    # For a static star in equilibrium:
    # - λ^r = 0 (no spatial connection correction needed initially)
...
def create_initial_data_interpolated(tov_solution, grid, background, eos,
    ...
    Args:
        ...
        grid: Engrenage evolution grid
        background: FlatSphericalBackground
        eos: Equation of state object (used for epsilon calculation)
        atmosphere: AtmosphereParams object
        interp_order: Lagrange interpolation order (default 11)
    ...
    # Interpolate PRIMITIVE hydro quantities with surface protection
    P_arr = np.zeros(N)
    rho_arr = np.zeros(N)
    ...
    for i in range(N):
        ...
        if is_interior:
            # Interpolate pressure and density from TOV fine solution (stencil not crossing R)
            P_loc = interpolate_tov_avoiding_surface(
                r_abs, tov_solution, 'P', interp_order, p_atm)
            P_loc = max(P_loc, p_atm)

            # Interpolate density directly from TOV solution
        ...
        # vr = 0 for static TOV star (no radial velocity)
        vr_arr[i] = 0.0

    # Extract gamma_rr from BSSN state for prim_to_cons conversion
    # We need the physical metric component γ_rr for the conversion
    # Reconstruct γ_rr from BSSN variables: γ_rr = e^(4φ) × γ̄_rr
    phi = bssn_state[idx_phi, :]
    hrr = bssn_state[idx_hrr, :]

    # Background metric ĝ_rr
    ghat_rr = background.hat_gamma_LL[:, i_r, i_r]

    # Conformal metric γ̄_rr = ĝ_rr + Re_rr × h_rr
    Re_rr = background.scaling_matrix[:, i_r, i_r]
...
def plot_initial_comparison(tov_solution, initial_state_2d, grid, hydro, output_dir="."):
    ...
    P_grid = prim['p']
    v_grid = prim['vr']
    alpha_grid = initial_state_2d[idx_lapse, :]
