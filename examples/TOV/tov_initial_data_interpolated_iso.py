...
def create_initial_data_iso(tov_solution, grid, background, eos,
    ...
    for i, r in enumerate(grid.r):
        ...
        if r_abs < 1e-8:
            # At origin, set values directly from TOV solution
            # exp4phi = 1.0 (since r_schw = r_iso at origin)
            exp4phi_arr[i] = 1.0
    ...
    print(f"    α:  min={np.min(alpha_arr):.6f}, max={np.max(alpha_arr):.6f}")
