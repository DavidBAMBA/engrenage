...
class TOVSolverIso:
    ...
    def solve(self, rho_central, r_max_iso=20.0, accuracy="high", verbose=True):
        ...
        if alpha_continuity_error > 1e-6 and verbose:
            warnings.warn(f"α discontinuity at surface: interior={alpha_interior_surface:.8f}, "
        ...
        if verbose:
            ...
            print(f"  α continuity = {alpha_continuity_error:.2e}")
...
def run_exterior_convergence_test(K, Gamma, rho_central, resolutions=None, save_path=None, verbose=True):
    ...
    Tests:
    1. Convergence of α discontinuity at surface
    ...
    for n in resolutions:
        ...
        solutions[n] = sol
        print(f"M = {sol['M_star']:.10f}, Δα = {sol['alpha_discontinuity']:.2e}, "
              f"Δψ = {sol['exp4phi_discontinuity']:.2e}")

    # Collect errors for convergence analysis
    errors = {
        'alpha_disc': [],           # α discontinuity at surface
    ...
    print("-"*70)
    print(f"{'Resolution':<12} | {'Δα surface':<15} | {'Δψ surface':<15} | {'|norm-1|':<12}")
    print("-"*70)
    for n in resolutions:
        sol = solutions[n]
    ...
    if alpha_disc < 1e-10:
        print(f"  α discontinuity at N={resolutions[-1]}: {alpha_disc:.2e} (EXCELLENT - machine precision)")
    elif alpha_disc < 1e-6:
        print(f"  α discontinuity at N={resolutions[-1]}: {alpha_disc:.2e} (GOOD - converging)")
    else:
        print(f"  α discontinuity at N={resolutions[-1]}: {alpha_disc:.2e} (CHECK - may indicate matching issue)")
    ...
    ax = axes[0, 0]
    ax.loglog(resolutions, errors['alpha_disc'], 'o-', linewidth=2, markersize=10, label='α discontinuity')
    ...
    ax.set_title('Surface α Values (o=num, x=analytical)', fontsize=12)
    ax.legend(fontsize=9)
    ax.grid(True, alpha=0.3)
...
if __name__ == "__main__":
    ...
    # Run exterior convergence test if requested
        ...
        results, fig_ext = run_exterior_convergence_test(
            args.K, args.Gamma, args.rho_central,
            resolutions=resolutions,
            save_path=exterior_path
        )
        if can_show:
            plt.show()
        else:
        ...
        print(f"  Grid points    : {solution.num_points}")
        print(f"  α continuity   : {solution.alpha_continuity_error:.2e}")
